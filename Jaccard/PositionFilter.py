...
sc = SparkContext("local","PositionFilter")
...
def position_filter((px,x),(py,y)):
	...
	return (intersection_cardinality+min(len(x)-len(rpx),len(y)-len(rpy)))/float(prefix_union_cardinality+max(len(x)-len(rpx),len(y)-len(rpy))) >= jaccard


candidates = candidates.filter(lambda (x,y):position_filter((doc1_prefix_map[x],doc1_map[x]),(doc2_prefix_map[y],doc2_map[y])))
candidates.saveAsTextFile('/output/ppc/')
...
candidates.saveAsTextFile('/output/pp/')